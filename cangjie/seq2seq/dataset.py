        ...
        with open(data_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                for i in range(len(chars) // steps + 1):
                    ...
                    outputs.insert(0, 5) # 5 for `<start>`
                    yield inputs, outputs

    dataset = tf.data.Dataset.from_generator(generator,
                                             output_shapes=((steps, ), (steps+1, )), # outputs add `<start>`
...
if __name__ == "__main__":
    ...
    # inputs: [batch_size, steps]  \in [0, 1, 2, ..., vocab_size+1], 0 for `pad`, 1 for `unk`
    # outputs: [batch_size, steps] \in [0, 1, 2, 3, 4, 5], 0 for `pad`, 5 for `<start>`
